...
function usage() {
    ...
    echo "  k8s-controller-01    run setup script for k8s-controller-01"
    echo "  k8s-controller-02    run setup script for k8s-controller-02"
    echo "  k8s-controller-03    run setup script for k8s-controller-03"
    echo "  k8s-worker-0*    run setup script for k8s-worker-0*"
}

case $1 in
    k8s-controller-01|k8s-controller-02|k8s-controller-03|k8s-worker-0*)
        ;;
    help)
        usage
        exit 255
...
case $1 in
    k8s-worker-0*)
        exit 0
        ;;
    k8s-controller-01|k8s-controller-02|k8s-controller-03)
        ;;
    *)
        exit 1
        ;;
...
case $1 in
    k8s-controller-01)
        ;;
    k8s-controller-02|k8s-controller-03)
        exit 0
        ;;
    *)
        exit 1
...
helm install argocd argo/argo-cd \
    ...
    --create-namespace \
    --namespace argocd \
    --values https://raw.githubusercontent.com/hsh00000/k8s/main/k8s-manifests/argocd-helm-chart-values.yaml
helm install argocd argo/argocd-apps \
    --version 0.0.1 \
    --values https://raw.githubusercontent.com/hsh00000/k8s/main/k8s-manifests/argocd-apps-helm-chart-values.yaml
...
# clone repo
git clone -b main https://github.com/hsh00000/k8s.git "$HOME"/k8s
